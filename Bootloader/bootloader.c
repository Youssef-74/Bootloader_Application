...
static uint8_t Perform_Flash_Erase(uint8_t Page_Number, uint8_t Number_Of_Pages){
	...
	if(Number_Of_Pages > CBL_FLASH_MAX_PAGE_NUMBER){
		...
		Sector_Validity_Status = INVALID_PAGE_NUMBER;
	}
	else{
...
#endif
				Remaining_Sectors = CBL_FLASH_MAX_PAGE_NUMBER - Page_Number;
				if(Number_Of_Pages > Remaining_Sectors){
					Number_Of_Pages = Remaining_Sectors;
				}
				else { /* Nothing */ }
				
				pEraseInit.TypeErase = FLASH_TYPEERASE_PAGES; /* Pages erase only */
				pEraseInit.PageAddress = (uint32_t)(0x08000000UL + (Page_Number * (0x400U)));        /* Initial FLASH sector to erase when Mass erase is disabled */
  ...
  HAL_Status = HAL_FLASH_Unlock();
	...
	if(HAL_Status != HAL_OK){
		...
		for(Payload_Counter = 0; Payload_Counter < Payload_Len; Payload_Counter++){
			...
			if(Payload_Counter % 2 == 0)
			{
				Data &= ~(0xFFFF); 	
				Data |= (Host_Payload[Payload_Counter]);
				continue;
			}
			else
			{
				Data = (Data & 0x00FF) | (Host_Payload[Payload_Counter] << (8U));
			}
			HAL_Status = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, Payload_Start_Address + (Payload_Counter - 1), Data);
			if(HAL_Status != HAL_OK){
				Flash_Payload_Write_Status = FLASH_PAYLOAD_WRITE_FAILED;
				break;
			}
